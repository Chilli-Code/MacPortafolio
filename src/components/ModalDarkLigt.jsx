...
const handleSelect = (option) => {
  ...
  if (document.startViewTransition) {
    ...
    transition.ready.then(() => {     
      ...
      maskedRect.setAttribute('fill', option === 'dark' ? '#000000' : '#ffffff');
  ...
  return (
    <div
      ref={popRef}
      className="modal-darklight flex flex-col gap-1 justify-center items-center"
      style={{ position: "absolute" }}
    >
      {/* Light Mode */}
      <div
        onClick={() => handleSelect("light")}
        className="w-full px-2 rounded-md transition-colors duration-300 hover:bg-[#007AFF] flex items-center justify-between cursor-pointer group"
      >
        <button className="text-black group-hover:text-white font-medium w-full text-left py-1 rounded">
          Light Mode
        </button>
        <Check
